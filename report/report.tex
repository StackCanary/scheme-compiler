...
\begin{verbatim}
(let  
 ([f (lambda (c) (cons (lambda (v) (set! c v)) (lambda () c)))])
 (let ([p (f 0)])  // Call f (0) to retrieve Pair and set C to 0
      ((car p) 12) // Call ((Car p) 12) to set C to 12
      ((cdr p))))  // Call ((Cdr p)) to retrieve C
\end{verbatim}
...
% in which ``set! c val'' is replaced with ``vector-set! c 0 val''. 
